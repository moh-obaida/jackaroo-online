...
export function DeckInfoPanel({ gameState, onShowDeckGuide, compact = false }: DeckInfoPanelProps) {
  const { t } = useApp();
  const discardCount = gameState.discardPile?.length ?? 0;
  const topDiscard =
    discardCount > 0 ? gameState.discardPile[discardCount - 1] : null;

  const boxClass = compact ? 'game-panel-compact space-y-2' : 'card-container-compact space-y-3';

  return (
    <div className={boxClass}>
      <div className="flex items-center justify-between gap-2">
        <h3 className="text-xs font-semibold text-cream-200/70">{t('game.discardPile')}</h3>
        <button type="button" onClick={onShowDeckGuide} className="btn-secondary text-[10px] px-2 py-1">
          {t('game.showDeck')}
        </button>
      </div>
      ...
      <div className="flex items-center justify-between gap-3 pt-1 border-t border-wood-800/50">
